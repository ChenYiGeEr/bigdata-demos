...
usage(){
  ...
  printf "\t%-10s  %-10s  %-2s \n" --server "server-name" "Name of web-api server"
...
BIN=`abs_path`
SERVER_NAME_SIMPLE=${SERVER_NAME/quality-/}
...
if [ "x${BASE_CONF_DIR}" != "x" ]; then
  ...
  copy_replace application-prod.yml
  copy_replace mybatis-config.xml
  copy_replace applicationContext-dataSource.xml
  copy_replace sparkProperties.json
  copy_replace env/*
fi
...
if [ "x${SQL_SOURCE_PATH}" != "x" ] && [ -f "${SQL_SOURCE_PATH}" ]; then
   ...
   if [ $? == 0 ]; then
      ...
      if [ $? == 0 ]; then
        ...
        read -p "Please input the db name(default: notice-demo)" DATABASE
